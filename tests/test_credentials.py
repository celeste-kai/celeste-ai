...
class TestGetCredentials:
    ...
    @pytest.mark.parametrize(
        "provider", [Provider.LOCAL, Provider.OLLAMA, Provider.TRANSFORMERS]
    )
    def test_get_unmapped_provider_raises(self, provider: Provider) -> None:
        """Test get_credentials raises for unmapped providers."""
        # Arrange
        creds = Credentials()  # type: ignore[call-arg]
...
class TestHasCredential:
    ...
    def test_unmapped_provider_raises(self, provider: Provider) -> None:
        """Test unmapped providers raise ValueError."""
        # Arrange
        creds = Credentials()  # type: ignore[call-arg]

        # Act & Assert
        with pytest.raises(ValueError, match="no credential mapping"):
            creds.has_credential(provider)
